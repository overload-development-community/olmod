...
namespace GameMod
{
    // Should be identical to the original except for the reticle. May need manual updating. This is lazy, I should be extending Impulse instead of PrimaryWeapon but there are typeof() checks that might break. Need to revise this slightly.
    public class MSImpulse : PrimaryWeapon
    {
        public MSImpulse()
        {
            displayName = "IMPULSE";
        ...
        public override void DrawHUDReticle(Vector2 pos, float m_alpha)
        {
            ...
            */
            ...
            vector.x = Mathf.Sin(num3) * num2;
            vector.y = (0f - Mathf.Cos(num3)) * num2;
            float a = m_alpha * (((!MPClassic.matchEnabled || GameManager.m_local_player.m_weapon_level[(int)GameManager.m_local_player.m_weapon_type] == WeaponUnlock.LEVEL_2A)) ? 1f : 0.25f);
            temp_pos.x = pos.x + vector.x;
            temp_pos.y = pos.y + vector.y;
            //UIManager.DrawSpriteUIRotated(temp_pos, 0.3f, 0.3f, num3 + (float)System.Math.PI / 2f, UIManager.m_col_ub2, m_alpha, 40);
            UIManager.DrawSpriteUIRotated(temp_pos, 0.4f, 0.4f, num3 + (float)System.Math.PI / 2f, UIManager.m_col_ub1, m_alpha, 43);
            UIManager.DrawSpriteUIRotated(temp_pos, 0.4f, 0.4f, num3 + (float)System.Math.PI / 2f, UIManager.m_col_ui4, m_alpha, 37);
            temp_pos.x = pos.x + vector.x * 1.4f;
            temp_pos.y = pos.y + vector.y * 1.4f;
            UIManager.DrawSpriteUIRotated(temp_pos, 0.4f, 0.4f, num3 + (float)System.Math.PI / 2f, UIManager.m_col_ui4, a, 37);
            num3 = 4.18879032f;
            vector.x = Mathf.Sin(num3) * num2;
            vector.y = (0f - Mathf.Cos(num3)) * num2;
            temp_pos.x = pos.x + vector.x;
