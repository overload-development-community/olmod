...
namespace GameMod
{
    public class Burstfire : PrimaryWeapon
	{
		BurstfireCoroutine bfc;

		public Burstfire()
        {
            displayName = "BURSTFIRE";
            Tag2A = "DX";
            Tag2B = "M";
        ...
        {
            base.SetShip(s);
			bfc = player.gameObject.AddComponent<BurstfireCoroutine>();
        }
        ...
        public override void Fire(float refire_multiplier)
        {
            ...
            {
				player.c_player_ship.m_refire_time += (player.m_weapon_level[(int)player.m_weapon_type] < WeaponUnlock.LEVEL_1) ? 0.78f : 0.66f;
				bfc.StartCoroutine(bfc.BFCoroutine(this, (player.m_weapon_level[(int)player.m_weapon_type] < WeaponUnlock.LEVEL_1) ? 0.13f : 0.10f));
			}
        }
		...
		{
			MPSniperPackets.MaybePlayerFire(rot: player.c_player_ship.c_transform.localRotation,
				player: player, type: (ProjPrefab)projprefab, pos: player.c_player_ship.m_muzzle_center.position, strength: 0f, upgrade_lvl: player.m_weapon_level[(int)player.m_weapon_type], no_sound: false, slot: 0);
			player.PlayCameraShake(CameraShakeType.FIRE_DRILLER, 1f, 1f);
			player.UseAmmo(2);
		}

		public class BurstfireCoroutine : MonoBehaviour
		{
			public IEnumerator BFCoroutine(Burstfire bf, float time)
            {
				bf.FireBurst();
		...
		// Currently this won't get called since we're reusing proj_driller, not a custom projectile. It'll call Driller.ProjectileFire() instead (which this is duplicated from)
